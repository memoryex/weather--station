...
; Inicializuojame serial jungtį iškart startuojant AHK
InitSerialPort()
OnExit(CloseSerialPort)
...
RestartBluetooth() {
    ...
    try {
        ...
        psCmd := Format('powershell.exe -Command "Start-Process powershell -Verb RunAs -ArgumentList ''-Command {1}''"', psInnerCmd)
        Run(psCmd, , "Hide")
...
InitSerialPort() {
    global g_hPort, COM_PORT, COM_BAUD
    if (g_hPort != -1 && g_hPort != 0) {
        DllCall("CloseHandle", "Ptr", g_hPort)
        g_hPort := -1
    }
...
CloseSerialPort(ExitReason, ExitCode) {
    global g_hPort
    if (g_hPort != -1 && g_hPort != 0) {
        DllCall("CloseHandle", "Ptr", g_hPort)
        g_hPort := -1
...
RelayPulse() {
    ...
    res := DllCall("WriteFile", "Ptr", g_hPort, "Ptr", VarOut, "UInt", 1, "Ptr", bytesWritten, "Ptr", 0)
    if (!res) {
        ; Jei rašymas nepavyko (pvz., atsijungė USB), bandom inicializuoti vėl ir pakartoti
        InitSerialPort()
        if (g_hPort != -1 && g_hPort != 0) {
            DllCall("WriteFile", "Ptr", g_hPort, "Ptr", VarOut, "UInt", 1, "Ptr", bytesWritten, "Ptr", 0)
        }
    }
