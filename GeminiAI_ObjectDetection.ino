...
size_t decode_base64(const char* input, uint8_t* output) {
    size_t len = strlen(input); if (len % 4 != 0) return 0;
    size_t out_len = len / 4 * 3;
    if (input[len - 1] == '=') out_len--; if (input[len - 2] == '=') out_len--;
...
String getCurrentTime() {
    struct tm timeinfo; if (!getLocalTime(&timeinfo)) return "Time Error";
    char buffer[30]; strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", &timeinfo);
...
void speakText(String text) {
    ...
    int httpCode = http.POST(payload);
    if (httpCode == 200) {
        String response = http.getString(); DynamicJsonDocument doc(60000); deserializeJson(doc, response);
...
void detectObjects() {
    ...
    String base64Image = base64_encode(fb->buf, fb->len); esp_camera_fb_return(fb);
    HTTPClient http;
    String url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=" + String(GEMINI_API_KEY);
    http.begin(url); http.setTimeout(30000); http.addHeader("Content-Type", "application/json");
    ...
    int httpCode = http.POST(payload);
    if (httpCode == 200) {
        String response = http.getString(); DynamicJsonDocument doc(16384); deserializeJson(doc, response);
        if (doc.containsKey("candidates") && doc["candidates"].size() > 0) {
            const char* aiText = doc["candidates"][0]["content"]["parts"][0]["text"];
            if (aiText) {
                String desc = String(aiText); int sig = parseSignal(desc);
                Serial.println("\n[AI] " + desc);
                sendDataToFirebase(desc, getCurrentTime(), base64Image, sig);
                speakText(desc);
            }
        }
    } else { Serial.printf("[Gemini] Error %d\n", httpCode); }
    http.end();
}
