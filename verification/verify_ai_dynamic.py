...
def run():
    with sync_playwright() as p:
        ...
        # Check for dynamic break rows
        rows = page.locator("#customBreaksContainer .break-row")
        count = rows.count()
        print(f"Found {count} break config rows.")
        if count == 5:
            print("PASS: Correct number of break configuration rows.")
        else:
            print("FAIL: Incorrect number of rows.")

        # Screenshot the AI tab with expanded table and new config
        screenshot_path = "/home/jules/verification/gd_ai_manual_config_final.png"
